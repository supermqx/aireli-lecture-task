...
router.get('/me', protect, async (req, res) => { const user = await findUserById(req.body.user as string); if (user) { res.json({ user }); } else { res.status(404).json({ message: 'User not found' }); }});
